...
export default function BenchmarkCard({ blueprint: bp }: BenchmarkCardProps) {
  ...
  const refs = (bp as any).references as Array<{title: string; url?: string}> | undefined;
  const paperRef = refs && refs.length > 0 ? refs[0] : null;
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
            <div className="flex-grow min-w-0">
                <div className="flex items-center mb-2">
                    <Icon name="book-open" className="w-6 h-6 mr-2.5 text-primary flex-shrink-0" />
                ...
                {bp.author && (
                  <div className="mb-2 flex flex-wrap items-center gap-2">
                    ...
                    {/* Paper link */}
                    {paperRef && paperRef.url && (
                      <a
                        href={paperRef.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 rounded-full bg-blue-500/10 dark:bg-blue-400/10 px-2.5 py-0.5 border border-blue-500/20 dark:border-blue-400/20 text-xs font-medium text-blue-700 dark:text-blue-300 hover:bg-blue-500/20 dark:hover:bg-blue-400/20 transition-colors"
                        title={paperRef.title || 'View paper'}
                      >
                        <Icon name="file-text" className="w-3.5 h-3.5" />
                        <span className="max-w-[200px] truncate">{paperRef.title || 'Paper'}</span>
                        <Icon name="external-link" className="w-3 h-3 opacity-60" />
                      </a>
                    )}
                  </div>
                )}
                {/* Paper link when no author */}
                {!bp.author && paperRef && paperRef.url && (
                  <div className="mb-2">
                    <a
                      href={paperRef.url}
                      target="_blank"
