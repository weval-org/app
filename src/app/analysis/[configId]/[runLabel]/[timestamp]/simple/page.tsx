...
import { SimpleClientPage } from './SimpleClientPage';
...
const getComparisonData = cache(async (params: ThisPageProps['params']): Promise<ComparisonDataV2> => {
  ...
  try {
    ...
    if (core) {
      console.log(`[Simple Page Fetch] Using core artefact for ${configId}/${runLabel}/${timestamp}`);
    ...
    if (!jsonData) {
      console.log(`[Simple Page Fetch] Data not found for file: ${fileName}`);
      notFound();
    }

    console.log(`[Simple Page Fetch] Using full data from storage for ${configId}/${runLabel}/${timestamp}`);
    return jsonData as ComparisonDataV2;

  } catch (error) {
    console.error(`[Simple Page Fetch] Failed to get comparison data for ${configId}/${runLabel}/${timestamp}:`, error);
    notFound();
  }
});

export default async function SimpleAnalysisPage(props: ThisPageProps) {
