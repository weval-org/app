...
import { TextualClientPage } from './TextualClientPage';
...
const getComparisonData = cache(async (params: ThisPageProps['params']): Promise<ComparisonDataV2> => {
  ...
  try {
    ...
    if (core) {
      console.log(`[Textual Page Fetch] Using core artefact for ${configId}/${runLabel}/${timestamp}`);
    ...
    if (!jsonData) {
      console.log(`[Textual Page Fetch] Data not found for file: ${fileName}`);
      notFound();
    }

    console.log(`[Textual Page Fetch] Using full data from storage for ${configId}/${runLabel}/${timestamp}`);
    return jsonData as ComparisonDataV2;

  } catch (error) {
    console.error(`[Textual Page Fetch] Failed to get comparison data for ${configId}/${runLabel}/${timestamp}:`, error);
    notFound();
  }
});

export default async function TextualAnalysisPage(props: ThisPageProps) {
