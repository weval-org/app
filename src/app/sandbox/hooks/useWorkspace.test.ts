...
describe('useWorkspace', () => {
    ...
    test('should load local files for anonymous users and create a default if none exist', () => {
        mockLoadFilesFromLocalStorage.mockReturnValue([]);
        const defaultBlueprint: ActiveBlueprint = { name: 'default.yml', path: 'local/default.yml', sha: '1', content: 'default content', isLocal: true };
    ...
    test('handleSave should save a modified local file to localStorage', async () => {
        const localFile: ActiveBlueprint = { name: 'local.yml', path: 'local/local.yml', sha: 'local-sha', content: 'original content', isLocal: true };
    ...
    test('handleSave should update a modified GitHub file via API', async () => {
        const remoteFile: ActiveBlueprint = { name: 'remote.yml', path: 'gh/remote.yml', sha: 'remote-sha', content: 'original content', isLocal: false };
    ...
    test('createBlueprint should create a new local file for logged-in users', async () => {
        const remoteFile: BlueprintFile = { name: 'remote-only.yml', path: 'gh/remote.yml', sha: 'remote-sha', isLocal: false };
    ...
    test('deleteBlueprint should remove a local file', async () => {
        const localFile: BlueprintFile = { name: 'local-to-delete.yml', path: 'local/delete.yml', sha: 'del-sha', isLocal: true };
    ...
    test('deleteBlueprint should remove a GitHub file and activate another', async () => {
        const remoteFileToDelete: BlueprintFile = { name: 'remote-to-delete.yml', path: 'gh/delete.yml', sha: 'del-sha', isLocal: false };
        const otherFileToRemain: BlueprintFile = { name: 'other.yml', path: 'gh/other.yml', sha: 'other-sha', isLocal: false };
    ...
    describe('runEvaluation', () => {
        const blueprint: ActiveBlueprint = { name: 'run.yml', path: 'local/run.yml', sha: 'run-sha', content: 'content', isLocal: true };
    ...
    describe('createPullRequest', () => {
        const remoteFile: ActiveBlueprint = { name: 'remote.yml', path: 'gh/remote.yml', sha: 'remote-sha', content: 'content', isLocal: false };
    ...
    test('loadFile should prompt user if the current file is dirty', async () => {
        const file1: ActiveBlueprint = { name: 'file1.yml', path: 'local/file1.yml', sha: '1', content: 'content1', isLocal: true };
        const file2: BlueprintFile = { name: 'file2.yml', path: 'local/file2.yml', sha: '2', isLocal: true };
    ...
    test('should merge local and remote files for logged-in users', async () => {
        const localFile: BlueprintFile = { name: 'local-only.yml', path: 'local/local.yml', sha: 'local-sha', isLocal: true };
        const remoteFile: BlueprintFile = { name: 'remote-only.yml', path: 'gh/remote.yml', sha: 'remote-sha', isLocal: false };
        ...
        await waitFor(() => {
            expect(result.current.files).toHaveLength(2);
            expect(result.current.files.some(f => f.name === 'local-only.yml')).toBe(true);
            expect(result.current.files.some(f => f.name === 'remote-only.yml')).toBe(true);
        });
    });
});