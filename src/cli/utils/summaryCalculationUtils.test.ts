...
const mockRun = (timestamp: string, perModelScores: Record<string, number | null>, temp: number = 0): EnhancedRunInfo => ({
  runLabel: 'test-run-label',
  timestamp,
  fileName: `test-run-label_${timestamp}_comparison.json`,
  temperature: temp,
  perModelHybridScores: new Map(Object.entries(perModelScores).map(([k, v]) => [k, { average: v, stddev: null }])),
});
...
describe('calculatePotentialModelDrift', () => {
  ...
  it('should detect significant drift for a common model', () => {
    const configs: EnhancedComparisonConfigInfo[] = [{
      ...
      runs: [
        mockRun('2024-07-01T12:00:00Z', { 'model-a': 0.8, 'model-b': 0.9 }),
        mockRun('2024-07-03T12:00:00Z', { 'model-a': 0.6, 'model-b': 0.91 }),
      ],
  ...
  it('should return null if no drift is significant', () => {
    const configs: EnhancedComparisonConfigInfo[] = [{
      ...
      runs: [
        mockRun('2024-07-01T12:00:00Z', { 'model-a': 0.8, 'model-b': 0.9 }),
        mockRun('2024-07-03T12:00:00Z', { 'model-a': 0.81, 'model-b': 0.91 }),
      ],
      latestRunTimestamp: '2024-07-03T12:00:00Z',
    }];
    const result = calculatePotentialModelDrift(configs);
  ...
  it('should return null if time difference is less than the threshold', () => {
    const configs: EnhancedComparisonConfigInfo[] = [{
      ...
      configTitle: 'Config 1',
      runs: [
        mockRun('2024-07-01T12:00:00Z', { 'model-a': 0.8 }),
        mockRun('2024-07-01T18:00:00Z', { 'model-a': 0.6 }), // Only 6 hours later
      ],
  ...
  it('should ignore runs with non-zero temperature', () => {
    const configs: EnhancedComparisonConfigInfo[] = [{
      ...
      configTitle: 'Config 1',
      runs: [
        mockRun('2024-07-01T12:00:00Z', { 'model-a': 0.8 }, 0),
        mockRun('2024-07-03T12:00:00Z', { 'model-a': 0.6 }, 0.7), // This run should be excluded
      ],
      latestRunTimestamp: '2024-07-03T12:00:00Z',
    }];
    const result = calculatePotentialModelDrift(configs);
  ...
  it('should not detect drift if a model is not common to all runs', () => {
    const configs: EnhancedComparisonConfigInfo[] = [{
      ...
      configTitle: 'Config 1',
      runs: [
        mockRun('2024-07-01T12:00:00Z', { 'model-a': 0.8, 'model-b': 0.9 }), // model-b is here
        mockRun('2024-07-03T12:00:00Z', { 'model-a': 0.6, 'model-c': 0.9 }), // but not here
      ],
      latestRunTimestamp: '2024-07-03T12:00:00Z',
    }];
    const result = calculatePotentialModelDrift(configs);
  ...
  it('should return null if no models are common across all runs', () => {
    const configs: EnhancedComparisonConfigInfo[] = [{
      ...
      runs: [
        mockRun('2024-07-01T12:00:00Z', { 'model-a': 0.8 }),
        mockRun('2024-07-03T12:00:00Z', { 'model-b': 0.6 }),
      ],
      latestRunTimestamp: '2024-07-03T12:00:00Z',
    }];
    const result = calculatePotentialModelDrift(configs);
  ...
  it('should pick the model with the most significant drift across multiple configs', () => {
    const configs: EnhancedComparisonConfigInfo[] = [
      { // Config 1 has a model with 0.2 drift
        ...
        runs: [
          mockRun('2024-07-01T12:00:00Z', { 'model-a': 0.8 }),
          mockRun('2024-07-03T12:00:00Z', { 'model-a': 0.6 }),
        ],
        latestRunTimestamp: '2024-07-03T12:00:00Z',
      },
      { // Config 2 has a model with 0.3 drift
        ...
        runs: [
          mockRun('2024-07-01T12:00:00Z', { 'model-b': 0.9 }),
          mockRun('2024-07-03T12:00:00Z', { 'model-b': 0.6 }),
        ],
        latestRunTimestamp: '2024-07-03T12:00:00Z',
      }
  ...
  it('should not fail if perModelHybridScores is null or undefined for a run', () => {
      ...
      runs: [
        mockRun('2024-07-01T12:00:00Z', { 'model-a': 0.8 }),
        { ...mockRun('2024-07-03T12:00:00Z', {}), perModelHybridScores: undefined },
      ],
      latestRunTimestamp: '2024-07-03T12:00:00Z',
    }];
     const result = calculatePotentialModelDrift(configs);
